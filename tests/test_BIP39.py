...
SEED_PASSWORD = "TREZOR"


def test_failures():
    validateMnemonic = "sleep kitten"
    with pytest.raises(ValueError) as exc_info1:
        bip39_mnemonic_to_seed(validateMnemonic)
    assert str(exc_info1.value) == "Mnemonic has not the right words size"
    validateMnemonic = "sleep kitten sleep kitten sleep kitten"
    with pytest.raises(ValueError) as exc_info2:
        bip39_mnemonic_to_seed(validateMnemonic)
    assert str(exc_info2.value) == "Mnemonic has not the right words size"
    ...
    with pytest.raises(ValueError) as exc_info3:
        bip39_mnemonic_to_seed(validateMnemonic)
    assert str(exc_info3.value) == "Mnemonic has not the right words size"
    validateMnemonic = "turtle front uncle idea crush write shrug there lottery flower risky shell"
    with pytest.raises(ValueError) as exc_info4:
        bip39_mnemonic_to_seed(validateMnemonic)
    assert str(exc_info4.value) == "Mnemonic is not from wordlist"
    validateMnemonic = (
        "sleep kitten sleep kitten sleep kitten sleep kitten sleep kitten sleep kitten"
    )
    with pytest.raises(ValueError) as exc_info:
        bip39_mnemonic_to_seed(validateMnemonic)
    assert str(exc_info.value) == "BIP39 Checksum is invalid for this mnemonic"


@pytest.mark.parametrize("test_data", BIP39_TESTS_DATA)
def test_strict_bip39(test_data):
    entropy = bytes.fromhex(test_data[0])
    mnemonic = test_data[1]
    seed = bytes.fromhex(test_data[2])
    assert entropy_to_mnemonic(entropy) == mnemonic
    assert bip39_mnemonic_to_seed(mnemonic, SEED_PASSWORD) == seed


@pytest.mark.parametrize("test_data", BIP39_FREE)
def test_free_bip39(test_data):
    """No wordlist : mnemonic can be whatever string, no checksum"""
    entropy = bytes.fromhex(test_data[0])
    mnemonic = test_data[1]
    seed = bytes.fromhex(test_data[2])
