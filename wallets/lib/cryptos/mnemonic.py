...
def normalize_text(seed):
    ...
    seed = "".join(
        [
            ...
            if not (seed[i] in whitespace and is_CJK(seed[i - 1]) and is_CJK(seed[i + 1]))
        ]
    )
    return seed
...
def words_mine(
    prefix, entbits, satisfunction, wordlist=wordlist_english, randombits=random.getrandbits
):
