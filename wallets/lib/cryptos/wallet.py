...
class HDWallet(object):
    ...
    def export_privkeys(self, password=None):
        ...
        return {
            "receiving": {
                addr: self.privkey(addr, password=password) for addr in self.receiving_addresses
            },
            "change": {
                addr: self.privkey(addr, password=password) for addr in self.change_addresses
            },
        }
    ...
    def details(self, password=None):
        return {
            "type": "%s %s"
            % ("Electrum" if self.keystore.electrum else "BIP39", self.keystore.xtype),
