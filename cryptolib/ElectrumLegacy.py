...
V2_LIST_LEN = len(v2_words)
V2_LIST_LEN_POW2 = V2_LIST_LEN * V2_LIST_LEN


def read_v2_words(words):
    """Decode 3 v2 words"""
    cs_res = v2_words.index(words[0])
    w2 = v2_words.index(words[1])
...
def decode_old_mnemonic(words):
    """Old electrum mnemonic string to seed"""
    words_list = words.split(" ")
    res = b""
    for i in range(len(words_list) // 3):
        res += read_v2_words(words_list[3 * i : 3 * i + 3])
    return res
