...
class BaseCoin(object):
    ...
    def mktx(self, *args):
        ...
        if any(
            isinstance(i, dict) and (i.get("segwit", False) or i.get("new_segwit", False))
            for i in ins
        ):
            if not self.segwit_supported:
    ...
    def preparesignedtx(
        self, privkey, to, value, fee=10000, change_addr=None, segwit=False, addr=None
    ):
        """
        ...
        """
        return self.preparesignedmultitx(
            privkey, to + ":" + str(value), fee, change_addr=change_addr, segwit=segwit, addr=addr
        )
    ...
    def sendmultitx(self, privkey, *args, change_addr=None, segwit=False, addr=None):
        ...
        tx = self.preparesignedmultitx(
            privkey, *args, change_addr=change_addr, segwit=segwit, addr=addr
        )
    ...
    def inspect(self, tx):
        ...
        osum = 0
        for _out in tx["outs"]:
            outs.append({"address": self.scripttoaddr(_out["script"]), "value": _out["value"]})
            osum += _out["value"]
